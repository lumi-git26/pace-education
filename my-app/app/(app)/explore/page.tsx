...
import CourseModal from "@/components/CourseModal"; // ĐÃ IMPORT COURSE MODAL VÀO ĐÂY
...
export default function ExplorePage() {
  ...
  // State lưu Khóa học cần xem trước để bật Modal
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      // 1. CHẶN KHÓA HỌC DRAFT (Lấy những khóa published)
      ...
      // 2. CHẶN LUÔN CẢ LỚP HỌC (CLASSROOMS) DRAFT VÀO ĐÂY NÈ!
      const { data: classRows } = await supabase
        .from("classrooms")
        .select("id, title, subject_code, cohort_label, status, profiles(full_name)")
        .eq("status", "published");
  ...
  function handleSearchSubmit() {
    ...
    // Tự động cuộn mượt mà xuống khu vực lưới Course
  ...
  return (
    <div className="relative w-full min-h-screen">
      ...
      <div className="relative z-10 pt-[88vh] px-6 w-full max-w-[1200px] mx-auto pb-32">
        <div ref={resultsAreaRef} className="scroll-mt-[100px]">
      ...
      {previewCourse && (
        <CourseModal 
          ...
          onEnrolled={() => {
            console.log("Enrolled via Modal from Suggestions");
          }}
        />
      )}
      {/* ========================================================= */}
